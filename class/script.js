...
// Run filters when both classes and access list are loaded
function runFiltersWhenReady() {
    if (!coursesGrid) return;

    if (!classesLoaded || !accessListLoaded) {
        if (!classesLoaded && !accessListLoaded) {
            coursesGrid.innerHTML = '<div class="loading">Loading classes and checking access...</div>';
        } else if (!classesLoaded) {
            coursesGrid.innerHTML = '<div class="loading">Loading classes...</div>';
        } else {
            coursesGrid.innerHTML = '<div class="loading">Checking your access permissions...</div>';
        }
        return;
    }

    // Both are loaded, now run the filter
    filterClasses();
}
...
function filterClasses() {
    ...
    // Access filtering - only show classes user has access to
    filtered = filtered.filter(classItem => 
        userAccessList.includes(String(classItem.id))
    );
