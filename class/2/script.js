...
let userAccessPermissions = []; // Store user's access permissions from Firestore
...
// Check if user has access to classes and store permissions
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (accessArray.length > 0) {
                ...
                userAccessPermissions = accessArray;
                showAuthAlert(`Welcome! You have access to: ${accessArray.join(', ')}`, 'success');
                
                // Apply access-based filtering to classes
                filterClassesByAccess();
...
// Filter classes based on user's Firestore access permissions
function filterClassesByAccess() {
    if (userAccessPermissions.length === 0) {
        // If no access permissions, show no classes
        filteredClasses = [];
    } else {
        // Filter classes based on user's access permissions
        filteredClasses = mockClasses.filter(classItem => {
            // Check if user has access to this class based on various criteria
            // You can modify this logic based on how your access control works
            
            // Option 1: Access by class ID
            if (userAccessPermissions.includes(classItem.id.toString())) {
                return true;
            }
            
            // Option 2: Access by course type
            if (userAccessPermissions.includes(classItem.courseType)) {
                return true;
            }
            
            // Option 3: Access by subject
            if (userAccessPermissions.includes(classItem.subject)) {
                return true;
            }
            
            // Option 4: Access by grade
            if (userAccessPermissions.includes(classItem.grade)) {
                return true;
            }
            
            // Option 5: Access by class title (partial match)
            for (let permission of userAccessPermissions) {
                if (classItem.title.toLowerCase().includes(permission.toLowerCase())) {
                    return true;
                }
            }
    ...
    // Apply additional filters (grade, course, subject dropdowns)
    filterClasses();
}
    ...
    try {
        ...
        mockClasses = [
            ...
            },
            {
                ...
                image: "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1�YWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1000&q=80"
            },
            {
                id: 20,
            ...
            {
                id: 83,
                title: "দুরন্ত প্রয়াস 5.0s Restart ( 2nd Timer Medical Batch )",
                subject: "Physics",
                grade: "Admission 25",
                courseType: "RTDS",
        ...
        // Initially set all classes, will be filtered by user access
        filteredClasses = [...mockClasses];

        // Simulate loading delay
        setTimeout(() => {
            // If user is already authenticated, apply access filtering
            if (currentUser && userAccessPermissions.length > 0) {
                filterClassesByAccess();
            } else {
                renderClasses();
            }
...
// Enhanced filter classes function with access control
function filterClasses() {
    // Start with classes that user has access to
    let baseClasses = mockClasses.filter(classItem => {
        // If no user access permissions set, allow all classes
        if (userAccessPermissions.length === 0) {
            return true;
        }
...
function createClassCard(classItem) {
    return `
        <div class="course-card">
            <img
                class="course-image"
                src="${classItem.image || 'https://images.unsplash.com/photo-1481627834876-b7833e8f5570?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80'}"
                alt="${classItem.title}"
                loading="lazy"
                onerror="this.src='https://images.unsplash.com/photo-1481627834876-b7833e8f5570?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80'"
            />
            <div class="course-content">
                ...
                <div class="course-meta">
                    ...
                    <div class="meta-item">
                        ...
                        <span>${classItem.courseType || 'Course'}</span>
                    </div>
                    <div class="meta-item">
...
function renderClasses() {
    ...
    if (filteredClasses.length === 0) {
        const message = currentUser 
            ? 'No classes found that you have access to with the selected filters.'
            : 'No classes found for the selected filters.';
        coursesGrid.innerHTML = `<div class="loading">${message}</div>`;
        return;
    }
